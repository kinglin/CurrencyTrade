...
class sfsupport:
    ...
    def fuzzify(self):
        self.error_sg_d = ctrl.Antecedent(np.arange(0, 1.01, 0.001), 'error_sg_d')

        self.error_sg_d['low'] = fuzz.trapmf(self.error_sg_d.universe, [0, 0, 0.075, 0.15])
        self.error_sg_d['medium'] = fuzz.trapmf(self.error_sg_d.universe, [0.05, 0.2, 0.2, 0.35])
        self.error_sg_d['high'] = fuzz.trapmf(self.error_sg_d.universe, [0.25, 0.325, 100, 100])

        self.error_p_d = ctrl.Antecedent(np.arange(0, 1.01, 0.001), 'error_p_d')

        self.error_p_d['low'] = fuzz.trapmf(self.error_p_d.universe, [0, 0, 0.15, 0.3])
        self.error_p_d['medium'] = fuzz.trapmf(self.error_p_d.universe, [0.1, 0.4, 0.4, 0.7])
        self.error_p_d['high'] = fuzz.trapmf(self.error_p_d.universe, [0.5, 0.65, 100, 100])

        self.error_p_sg = ctrl.Antecedent(np.arange(0, 1.01, 0.001), 'error_p_sg')

        self.error_p_sg['low'] = fuzz.trapmf(self.error_p_sg.universe, [0, 0, 0.075, 0.15])
        self.error_p_sg['medium'] = fuzz.trapmf(self.error_p_sg.universe, [0.05, 0.2, 0.2, 0.35])
        self.error_p_sg['high'] = fuzz.trapmf(self.error_p_sg.universe, [0.25, 0.325, 100, 100])
        ...
        if mytrade.DEBUGLEVEL == 1:
            self.error_sg_d.view()
            self.error_p_d.view()
            self.error_p_sg.view()
    ...
    def inference(self):
        self.rule11 = ctrl.Rule(self.error_sg_d['low'], self.s_factor_sg_d['high'])
        self.rule12 = ctrl.Rule(self.error_sg_d['medium'], self.s_factor_sg_d['medium'])
        self.rule13 = ctrl.Rule(self.error_sg_d['high'], self.s_factor_sg_d['low'])

        self.rule21 = ctrl.Rule(self.error_p_d['low'], self.s_factor_p_d['high'])
        self.rule22 = ctrl.Rule(self.error_p_d['medium'], self.s_factor_p_d['medium'])
        self.rule23 = ctrl.Rule(self.error_p_d['high'], self.s_factor_p_d['low'])

        self.rule31 = ctrl.Rule(self.error_p_sg['low'], self.s_factor_p_sg['high'])
        self.rule32 = ctrl.Rule(self.error_p_sg['medium'], self.s_factor_p_sg['medium'])
        self.rule33 = ctrl.Rule(self.error_p_sg['high'], self.s_factor_p_sg['low'])
    ...
    def defuzzify(self, data):
        self.simu_sd.input['error_sg_d'] = data['MAPE_SG/D']
        self.simu_sd.compute()
        sf_sd = self.simu_sd.output['s_factor_sg_d']

        self.simu_pd.input['error_p_d'] = data['MAPE_P/D']
        self.simu_pd.compute()
        sf_pd = self.simu_pd.output['s_factor_p_d']

        self.simu_ps.input['error_p_sg'] = data['MAPE_P/SG']
